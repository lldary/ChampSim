...
template <typename T>
auto my_prefetcher::page_and_offset(T addr) -> std::pair<champsim::page_number, block_in_page>
{
  return std::pair{champsim::page_number{addr}, block_in_page{addr}};
}

bool my_prefetcher::check_cl_access(champsim::block_number v_addr)
{
...
bool my_prefetcher::check_cl_prefetch(champsim::block_number v_addr)
{
  auto [vpn, page_offset] = page_and_offset(v_addr);
  auto region = regions.check_hit(region_type{vpn});
